...
func (recv *ParameterModifier) AddValuesToExecuteFrame(
	newFrame *frame.Frame, preparedStatementInfo *PreparedStatementInfo,
	...
	if len(preparedStatementInfo.replacedTerms) > 0 {
		err := recv.addValuesToExecuteMessage(
			newFrame.Header.Version, newExecuteMsg, preparedStatementInfo, variablesMetadata, replacementTimeUuids)
		if err != nil {
			return nil, err
		}
	}
...
func (recv *ParameterModifier) addValuesToExecuteMessage(
	...
	preparedStatementInfo *PreparedStatementInfo, variablesMetadata *message.VariablesMetadata,
	...
	if len(executeMsg.Options.NamedValues) == 0 {
		...
		newPositionalValues, err = recv.addPositionalValuesForReplacedTerms(
			version, executeMsg.Options.PositionalValues, preparedStatementInfo, variablesMetadata, replacementTimeUuids)
		if err == nil {
...
func (recv *ParameterModifier) addValuesToBatchChild(
	...
	newPositionalValues, err := recv.addPositionalValuesForReplacedTerms(
		version, batchChild.Values, preparedStatementInfo, variablesMetadata, replacementTimeUuids)
	if err == nil {
...
func (recv *ParameterModifier) generateTimeUuids(preparedStatementInfo *PreparedStatementInfo) []*uuid.UUID {
	generatedUuids := make([]*uuid.UUID, 0, len(preparedStatementInfo.replacedTerms))
	for _, currentTerm := range preparedStatementInfo.replacedTerms {
...
func (recv *ParameterModifier) addPositionalValuesForReplacedTerms(
	...
	if preparedStatementInfo.containsPositionalMarkers {
		return recv.addPositionalValuesForReplacedPositionalMarkers(
			version, originalPositionalValues, preparedStatementInfo.replacedTerms, variablesMetadata, replacementTimeUuids)
	} else {
